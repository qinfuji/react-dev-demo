...
class Agent extends EventEmitter {
  ...
  addBridge(bridge: Bridge) {
    ...
    bridge.on('traceupdatesstatechange', value =>
    ...
    this.on('setInspectEnabled', data =>
      bridge.send('setInspectEnabled', data)
    );
  }
  ...
  _setContext({
    ...
    if (
      data &&
      data.updater &&
      typeof data.updater.setInContext === 'function'
    ) {
      // $FlowFixMe
      data.updater.setInContext(path, value);
    } else {
      console.warn(
        'trying to set context on a component that doesn\'t support it'
      );
    }
  }
